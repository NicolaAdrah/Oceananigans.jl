...
function attach_writers_velocities!(simulation; output_dir = "validation/open_boundaries/hydro_bc_output", time_int=10minutes)
...
function attach_writers_tracer!(simulation; output_dir = "validation/open_boundaries/hydro_bc_output", time_int=10minutes)
    if !isdir(output_dir); mkpath(output_dir); end
...
function make_movie_velocities(output_dir)
    ...
    lines!(axu, un); lines!(axv, vn); lines!(axw, wn); lines!(axη, ηn);
    ylims!(axw, (-1e-5, 1e-5)); ylims!(axη, (-0.1, 0.2)); ylims!(axu, (-0.1, 0.1))

    record(fig, joinpath(output_dir, "cu_hydro_bc_velocities.mp4"), 1:Nt) do i
        @info "frame $i / $Nt"
        n[] = i
    end

    return joinpath(output_dir, "cu_hydro_bc_velocities.mp4")
end


function make_movie_tracer(output_dir)
    ...
    record(fig, joinpath(output_dir, "cu_hydro_bc_tracer.mp4"), 1:Nt) do i
        @info "frame $i / $Nt"
        n[] = i
    end

    return joinpath(output_dir, "cu_hydro_bc_tracer.mp4")
end
...
function make_tracer_heatmap(output_dir)
    ...
    return joinpath(output_dir, "heatmap_tracer.mp4")
end
...
export attach_writers_velocities!, attach_writers_tracer!, make_movie_velocities, make_movie_tracer, make_tracer_heatmap, make_debug_tracer_and_u
