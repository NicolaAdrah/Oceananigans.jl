...
function attach_writers!(simulation; output_dir = "validation/open_boundaries/hydro_bc_output")
...
# --- Extra: tracer heatmap animation helper ---
...
function make_tracer_timeseries(output_dir)
    ts = FieldTimeSeries(joinpath(output_dir, "tracer.jld2"), "c")
    Nt = length(ts.times)
    times = ts.times ./ 3600  # seconds -> hours
    means = [mean(Array(interior(ts[i], :, 1, :))) for i in 1:Nt]

    fig = Figure(resolution = (800, 400))
    ax = Axis(fig[1,1], xlabel="time [hours]", ylabel="mean(c)")
    lines!(ax, times, means)
    save(joinpath(output_dir, "tracer_timeseries.png"), fig)
    return joinpath(output_dir, "tracer_timeseries.png")
end

# --- Debug utilities: check if flow & tracer evolve ---
function make_debug_tracer_and_u(output_dir)
    ...
    # Tracer center-of-mass in x (index units), averaged over z at y=1
    comx = Float64[]
    for i in 1:Nt
...
export attach_writers!, make_movie, make_tracer_heatmap, make_tracer_timeseries, make_debug_tracer_and_u
