...
function initialize_solution!(q, x, b, solver, args...)
   ...
   if any(!isfinite, parent(b))
       bad = findfirst(!isfinite, parent(b))
       ci = CartesianIndices(parent(b))[bad]
       @warn "Right-hand side b contains non-finite values!" first_index = Tuple(ci) first_value = parent(b)[bad]
   end
